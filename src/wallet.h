...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 QBIC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zQBIC send
enum ZerocoinSpendStatus {
    ZQBIC_SPEND_OKAY = 0,                            // No error
    ZQBIC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZQBIC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZQBIC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZQBIC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZQBIC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZQBIC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZQBIC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZQBIC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZQBIC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZQBIC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZQBIC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZQBIC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZQBIC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZQBIC_SPENT_USED_ZQBIC = 14,                      // Coin has already been spend
    ZQBIC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZQBIC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZQBICOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzQBICWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzQBICTracker> zpivTracker;
    ...
    void setZWallet(CzQBICWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzQBICTracker>(new CzQBICTracker(strWalletFile));
    }

    CzQBICWallet* getZWallet() { return zwalletMain; }
    ...
    /** zQBIC reset */
    boost::signals2::signal<void()> NotifyzQBICReset;
