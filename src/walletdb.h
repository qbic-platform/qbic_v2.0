...
public:
    ...
    bool WriteZQBICSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZQBICSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZQBICSeed_deprecated(uint256& seed);
    bool EraseZQBICSeed();
    bool EraseZQBICSeed_deprecated();

    bool WriteZQBICCount(const uint32_t& nCount);
    bool ReadZQBICCount(uint32_t& nCount);
