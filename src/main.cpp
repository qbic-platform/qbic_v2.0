...
#if defined(NDEBUG)
#error "QBICcoin cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zQBIC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zQBIC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zQBIC without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zQBIC spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zQBIC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zQBIC spend tx %s already in block %d",
            ...
            // Check that zQBIC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidyHalvingInterval = 525600; //1 year
    for (int i = nSubsidyHalvingInterval; i <= nHeight - nSubsidyHalvingInterval; i += nSubsidyHalvingInterval) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZQBICStake)
{
    int64_t ret = 0;
    ...
    ret = blockValue * 0.70; // 70 % goes to masternodes
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from QBICcoin
...
{
    RenameThread("qbiccoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZQBICMinted()
{
...
void RecalculateZQBICSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zQBIC supply
...
bool RecalculateQBICSupply(int nHeightStart)
{
...
bool UpdateZQBICSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateQBICSupply(Params().Zerocoin_StartHeight());
    }

    //Track zQBIC money supply in the block index
    if (!UpdateZQBICSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zQBIC supply for block=%s height=%d", __func__,
...
//  CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight + 1);
    ...
    if (pindex->pprev->nHeight > 16000) {
        nExpectedMint = GetBlockValue(pindex->pprev->nHeight + 1);
    }
    if (block.IsProofOfWork())
       nExpectedMint += nFees;
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert QBIC to zQBIC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            if (block.vtx[i].IsCoinStake())
                ...
                // Check for coin age.
        		// First try finding the previous transaction in database.
            if (chainActive.Tip()->nHeight > 150 ){
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zQBIC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zQBIC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zQBIC stake block is less than allowed start height", __func__);

    if (CZPivStake* zQBIC = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zQBIC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zQBIC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zQBIC stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zQBIC->GetValue()));
        if (nChecksum200 != zQBIC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zQBIC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZQBIC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zQBIC fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zQBIC mints and %d zQBIC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // QBICcoin: We use certain sporks during IBD, so check to see if they are
