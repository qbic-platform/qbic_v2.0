...
CzQBICWallet::CzQBICWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZQBICSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZQBICSeed_deprecated()) {
                    LogPrintf("%s: Updated zQBIC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzQBICWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZQBICCount(nCountLastUsed);
    else if (!walletdb.ReadZQBICCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzQBICWallet::Lock()
{
    seedMaster = 0;
}

void CzQBICWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzQBICWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZQBIC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzQBICWallet::LoadMintPoolFromDB()
{
...
void CzQBICWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzQBICWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzQBICWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzQBICWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZQBIC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzQBICWallet::SeedToZQBIC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzQBICWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzQBICWallet::GenerateDeterministicZQBIC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzQBICWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzQBICWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
