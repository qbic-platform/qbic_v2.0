...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::QBIC);
    ...
    if (!settings.contains("nAnonymizeqbiccoinAmount"))
        settings.setValue("nAnonymizeqbiccoinAmount", 1000);

    nAnonymizeqbiccoinAmount = settings.value("nAnonymizeqbiccoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeqbiccoinAmount"))
        SoftSetArg("-anonymizeqbiccoinamount", settings.value("nAnonymizeqbiccoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in qbiccoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeqbiccoinAmount:
            return QVariant(nAnonymizeqbiccoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeqbiccoinAmount:
            nAnonymizeqbiccoinAmount = value.toInt();
            settings.setValue("nAnonymizeqbiccoinAmount", nAnonymizeqbiccoinAmount);
            emit anonymizeqbiccoinAmountChanged(nAnonymizeqbiccoinAmount);
            break;
        case CoinControlFeatures:
