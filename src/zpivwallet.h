...
#ifndef QBICcoin_ZQBICWALLET_H
#define QBICcoin_ZQBICWALLET_H
...
public:
    CzQBICWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZQBIC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZQBIC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //QBICcoin_ZQBICWALLET_H
